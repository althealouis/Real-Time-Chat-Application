...
function Login() {
  ...
  const handleLogin = async (e) => {
    try {
      ...
      // console.log('Logged in from the client side', response.data);

      if(response.status === 200) {
        // console.log("response ok for login", response.data);
  ...
  return (
    <>
      ...
      <div className='bg-slate-900 h-screen flex items-center justify-center shadow-xl shadow-slate-100 mx-auto p-6'>
        <form className='w-80 mt-4 block text-black font-bold border border-sky-200 shadow-lg p-5 rounded-lg bg-slate-700' onSubmit={handleLogin}>
          <h1 className='text-3xl text-white font-bold p-5 text-center shadow-md bg-neutral-600 rounded-lg border mb-3'>Login</h1>
          <input 
            type="text" 
          ...
          <div className="font-medium flex justify-center p-3 text-white -mb-2">
            ...
            <Link to="/register" className="text-sky-300">Sign up!</Link>
          </div>
        </form>
      </div>
    </>
